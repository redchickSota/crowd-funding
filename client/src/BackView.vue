                ...
                <div class="container">
                    <div class="row">
                        <div class="tim-title">
                            ...
                            <div v-else="">
                                To view the projects you have backed you need to log in.
                            </div>
        ...
        methods: {
            getBackedProjects: function(){
                this.$http.get('http://csse-s365.canterbury.ac.nz:4708/api/v2/projects', {
                    params: {
                        'backer': parseInt(this.$cookie.get('user_id'))
                    }
                }).then(function(response) {


                    for (var i = 0; i < response.data.length; i++){
                        if(!(this.checkDuplicates.includes(response.data[i].id))){
                            this.checkDuplicates.push(response.data[i].id);
